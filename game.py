...
def display_character_info(player):
    ...
    base_attack_power = player.fighter.base_strength
    attack_power = player.fighter.strength
...
def check_level_up(player):
    ...
    if player.fighter.xp >= level_up_xp:
        ...
        # if allowed to add strength, then append Strength question to menu, set strength_menu to the next number in squence, and add 1 to size of menu
        strength_menu = optional_menu_item_add(m, 'Strength (+1 attack, from ' + str(player.fighter.strength) + ')', 
                                               (player.fighter.strength < (player.level+player.variables.strength_var)))
        defense_menu =  optional_menu_item_add(m, 'Defense (+1 defense, from ' + str(player.fighter.defense) + ')',
                                               (player.fighter.defense < (player.level+player.variables.defense_var)))
        reflex_menu = optional_menu_item_add(m, 'Reflex (+1 alertness, from ' + str(player.fighter.reflex) + ')', 
                                             (player.fighter.reflex < (player.level+player.variables.reflex_var)))
        ...
        choice = None
        while choice is None:  #keep asking until choice is made
        ...
        if choice == hp_menu:
            ...
            player.fighter.hp += 20
        elif choice == defense_menu:
            player.fighter.base_defense += 1
            player.fighter.hp += 20
        elif choice == reflex_menu:
...
def new_game():
    ...
    fighter_component = Fighter(hp=100, defense=1, strength=2, reflex=1, weapon_skill=1, shield_skill=1, speed_value=6, xp=0, death_function=player_death)
    variables_component = Variables(strength_var=0, defense_var=0, reflex_var=0, weapon_skill_var=0, shield_skill_var=0)
    player = Object(algebra.Location(0, 0), '@', 'player', libtcod.white, blocks=True, fighter=fighter_component, variables=variables_component)
    player.inventory = []
    player.level = 1
    player.game_state = 'playing'
    ...
    log.message('Welcome stranger to the the eternal, twisted and slimy realm of noodles', libtcod.red)
