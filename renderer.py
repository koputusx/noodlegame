...
def renderer_init():
    ...
    libtcod.console_init_root(config.SCREEN_WIDTH, config.SCREEN_HEIGHT, 'noodlegame', False)
...
def main_menu(new_game, play_game, load_game):
    ...
    while not libtcod.console_is_window_closed():
        ...
        libtcod.console_print_ex(
            ...
            libtcod.CENTER, 'INFINITE CAVERNS OF NOODLES')
        libtcod.console_print_ex(
            0, config.SCREEN_WIDTH/2, config.SCREEN_HEIGHT-2, libtcod.BKGND_NONE,
            libtcod.CENTER, 'By Kopi, with help of hkas and roguelike tutorial on roguebasin')
...
def draw_panel(player, pointer_location):
    ...
    libtcod.console_print_ex(
        ...
        '\nDefense ' + str(player.fighter.defense) + '\nReflex ' + str(player.fighter.reflex))
