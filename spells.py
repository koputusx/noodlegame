...
def cast_paralyze(actor):
    log.message('left-click to paralyze an enemy, or right click to cancel',
                libtcod.light_cyan)
    monster = _target_monster(actor, PARALYZE_RANGE)
    if monster is None:
        return 'cancelled'
    old_ai = monster.ai
    old_reflex = monster.fighter.reflex
    old_weapon_skill = monster.fighter.weapon_skill
    monster.ai = AI(ai.paralyzed_monster, ai.paralyzed_monster_metadata(old_ai, old_reflex, old_weapon_skill))
    monster.ai.set_owner(monster)
    log.message(monster.name + 
